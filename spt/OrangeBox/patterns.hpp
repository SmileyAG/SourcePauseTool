...
namespace patterns
{
	namespace engine
	{
		...
		PATTERNS(_Host_RunFrame,
		         ...
		         "4044",
		         "55 8B EC 83 EC 10 80 3D ?? ?? ?? ?? 00 53 56 57 74 17 E8 ?? ?? ?? ?? 83 F8 FE 74 0D 68");
		PATTERNS(
		    SV_Frame,
		    "5135",
	...
	namespace client
	{
		...
		PATTERNS(
		    ...
		    "4044",
		    "53 55 56 57 6A 14 8B F1 E8 ?? ?? ?? ?? 8B 4C 24 18 8B F8 0F B7 01 89 44 24 18 83 C4 04 DB 44 24 14");
		PATTERNS(
		    CViewEffects__Shake,
		    "4104-5135",
	...
	namespace server
	{
		...
		PATTERNS(
		    ...
		    "4044",
		    "8B 44 24 04 3B 05 ?? ?? ?? ?? 74 3E 3B 05 ?? ?? ?? ?? 75 1D 8B 81 94 09 00 00 6A 00 83 E0 DF 6A 02 89 81 94 09 00 00 E8 ?? ?? ?? ?? B0 01 C2 0C 00");
		PATTERNS(
		    GetStepSoundVelocities,
		    "5135",
		    "F6 81 00 01 00 00 02 75 24 80 B9 32 01 00 00 09 74 1B D9 ?? ?? ?? ?? ?? 8B ?? ?? ?? 8B ?? ?? ?? D9 18 D9 ?? ?? ?? ?? ?? D9 19 C2 08 00",
		    "4044",
		    "8B 86 ?? ?? ?? ?? 33 DB 83 C4 04 83 FA 09 0F 94 C3 A8 02 75 18 85 DB 75 14 D9 05 B8 0E 3E 22 C7 44 24 10 00 00 5C 43 89 5C 24 14 EB 16");
		PATTERNS(
		...
		PATTERNS(SetPredictionRandomSeed, "4044-5135", "8B 44 24 ?? 85 C0 75 ?? C7 05 ?? ?? ?? ?? FF FF FF FF");
